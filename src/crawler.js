...
export class Crawler {
  ...
  async _generateRedirects() {
    ...
    const htaccessLines = [
      ...
      '# Default handlers, access control, caching, MIME types, and redirects',
      '',
      '# Prefer the PHP redirect fallback, then static HTML documents.',
      'DirectoryIndex index.php index.html index.htm',
      '',
      '# Keep crawl state available for --resume, but never serve it publicly.',
      '<FilesMatch "^\\.?statify-state\\.json$">',
      '    Require all denied',
      '</FilesMatch>',
      '',
      ...
      '</IfModule>',
      '',
      '# Cache immutable assets while keeping HTML and crawl state fresh.',
      ...
      '    <FilesMatch "\\.(?:css|js|mjs|png|jpe?g|gif|svg|webp|ico|woff2?|ttf|otf|eot|wasm)$">',
  ...
  /**
   * Keep the state file after a crawl so it can be inspected or resumed.
   */
  async _clearState() {
    this.logger.info('Keeping crawl state file for future inspection or resume.');
  }
